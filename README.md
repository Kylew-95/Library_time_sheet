...
With this configuration, requests to `/api/*` are redirected to the Python function (`/.netlify/functions/library_excel`), and everything else is served by the React app. The frontend defaults to using the relative `/api` base at runtime and will automatically fall back to the direct Netlify function path if the rewrite is unavailable.
...
3. Confirm `/api` endpoints respond (e.g., `/api/staff`, `/api/profiles`, `/api/generate-timesheet`) and the built site loads from `build/`.
   - When running the Flask app directly instead of Netlify, set `REACT_APP_API_BASE=http://127.0.0.1:5000` so the frontend points to your local server instead of the Netlify function path. Leaving `REACT_APP_API_BASE` unset in production tells the frontend to try `/api` first and then fall back to `/.netlify/functions/library_excel`.

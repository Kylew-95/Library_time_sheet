...
TASK_CONFIG = {
    ...
    "SM":         {"roles": ["Scale 3"], "mandatory": 1, "full_name": "SM"},           # DISPLAY: SM, GREEN background
    "R":          {"roles": ["Scale 3"], "mandatory": 1, "full_name": "R"},            # DISPLAY: R
    "C":          {"roles": ["Scale 3"], "mandatory": 0, "full_name": "C (C/AV)"},     # DISPLAY: C (C/AV)
    "C+":         {"roles": ["Scale 3"], "mandatory": 0, "full_name": "C+ (A/T)"},    # DISPLAY: C+ (A/T)
...
def generate_schedule_data(staff_data, date_str):
    ...
    def sort_key(s):
        availability_priority = 1 if s.get("status", "Available") == "Available" else 2
    ...
    for hour in range(MIN_HOUR, MAX_HOUR):
        ...
        for s in staff_data:
            ...
            is_on_break = s.get("tea_slot", "") == current_time_str or s.get("setup_slot", "") == current_time_str
        ...
        scale3_staff = [s for s in available_staff_for_hour if s["role"] == "Scale 3"]
        
        for task in mandatory_tasks_to_assign:
            if scale3_staff:
                staff_to_assign = scale3_staff.pop(0) 
                pivot_schedule[staff_to_assign["name"]][current_time_str] = task
                tasks_assigned_in_hour.append(staff_to_assign["name"])

        # 3b. Randomize Remaining Tasks (1st, Res)
        remaining_staff = [s for s in available_staff_for_hour if s["name"] not in tasks_assigned_in_hour]
        random_tasks = [t for t in TASK_CONFIG if TASK_CONFIG[t]["mandatory"] == 0 and t not in ["Set Up", "T", "C", "C+", "R"]]
        
        for staff in remaining_staff:
            role = staff["role"]
            assignable_tasks = [t for t in random_tasks if role in TASK_CONFIG[t]["roles"]]
    ...
    pivot_df_data = []
    task_code_map = {k: v["full_name"] for k, v in TASK_CONFIG.items()} # Map internal code to full display name
    ...
    for staff in staff_data:
        ...
        if staff.get("status", "Available") != "Available":
            ...
            if staff["status"] == "Sick": status_display = "SICK"
            elif staff["status"] == "Annual Leave": status_display = "A/L"
